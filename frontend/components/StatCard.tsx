...
const ACCENTS: Record<NonNullable<Props["accent"]>, { stroke: string; fill: string }> = {
...
export default function StatCard({
  ...
  const data = (trend && trend.length > 0 ? trend : [0, 0]).map((v, i) => ({ i, v }));
    ...
    >
      ...
      <div className="mt-3 flex items-end justify-between gap-3">
        {hasDelta ? (
          ...
          <span className="text-[12px] text-[var(--text-tertiary)]">{deltaSuffix}</span>
        )}
