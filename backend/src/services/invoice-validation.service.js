...
const validateInvoicePayload = async (payload, context = {}) => {
  ...
  if (!customer_id && !vendor_id && !payload.customer_name) {
    errors.push({
      ...
      message: "Customer name (or a customer/vendor record) is required.",
    });
  }
  if (customer_id) {
    ...
    if (!customer) {
      ...
      warnings.push({
        ...
        message:
          "Customer GST number format may be invalid (expected 15 chars).",
      });
    }
  }
  if (vendor_id) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!item.product_id && !item.description) {
      errors.push({
        ...
        field: `items[${i}]`,
        message: "Description is required if product is not selected.",
      });
    }
    ...
    if (item.product_id) {
      ...
      if (!product) {
        ...
        if (price < product.price) {
          warnings.push({
            code: "PRICE_BELOW_ALLOWED",
            field: `items[${i}]`,
            message: `Price ${price} is below standard price ${product.price}.`,
          });
        }
      }
    }
    ...
    if (!ALLOWED_GST_RATES.includes(taxRate)) {
      warnings.push({
        ...
        field: `items[${i}].tax_rate`,
        message: `Tax rate ${taxRate}% is not a standard GST rate.`,
      });
    }
  ...
  if (
    frontendSubtotal !== undefined &&
    Number(frontendSubtotal) !== calculatedSubtotal
  ) {
    warnings.push({
      code: "FRONTEND_TOTAL_MISMATCH",
      message: `Frontend subtotal ${frontendSubtotal} mismatch with backend ${calculatedSubtotal}`,
    });
  }
  if (
    frontendGrandTotal !== undefined &&
    Number(frontendGrandTotal) !== calculatedGrandTotal
  ) {
    warnings.push({
      code: "FRONTEND_TOTAL_MISMATCH",
      message: `Frontend grand total ${frontendGrandTotal} mismatch with backend ${calculatedGrandTotal}`,
    });
  }
