...
const EDITABLE_FIELDS = [
  'name', 'description', 'sku', 'unit', 'category',
  'price', 'cost', 'tax_rate', 'stock', 'reorder_level', 'is_active'
];
...
router.post('/', requirePermission('inventory.create'), async (req, res, next) => {
  try {
    if (!req.body.name) return res.status(400).json({ success: false, message: 'name is required' });

    const insert = {};
    EDITABLE_FIELDS.forEach((f) => { if (req.body[f] !== undefined) insert[f] = req.body[f]; });

    const { data, error } = await supabase.from('products').insert([insert]).select().single();
    if (error) throw error;

    await createAuditLog({
      req, action: 'product_created', module: 'Inventory',
      entityType: 'product', entityId: String(data.id), newData: data
    });
...
router.put('/:id', requirePermission('inventory.update'), async (req, res, next) => {
  try {
    const patch = { updated_at: new Date().toISOString() };
    EDITABLE_FIELDS.forEach((f) => { if (req.body[f] !== undefined) patch[f] = req.body[f]; });
    ...
    await createAuditLog({
      req, action: 'product_updated', module: 'Inventory',
      entityType: 'product', entityId: String(data.id), oldData: existing, newData: data
    });
...
router.delete('/:id', requirePermission('inventory.delete'), async (req, res, next) => {
  try {
    const { data: existing } = await supabase.from('products').select('*').eq('id', req.params.id).maybeSingle();
    if (!existing) return res.status(404).json({ success: false, message: 'Product not found' });
    ...
    if (req.query.hard === 'true') {
      ...
      if (error) {
        ...
        return res.status(409).json({
          success: false,
          message: 'Product is referenced by existing documents. Use the default soft delete (is_active=false).',
          detail: error.message
        });
      }
      await createAuditLog({
        req, action: 'product_deleted', module: 'Inventory',
        entityType: 'product', entityId: String(existing.id), oldData: existing
      });
      return res.json({ success: true, message: 'Product permanently deleted', deleted: true });
    }
    ...
    await createAuditLog({
      req, action: 'product_deactivated', module: 'Inventory',
      entityType: 'product', entityId: String(data.id), oldData: existing, newData: data
    });
...
router.post('/:id/stock-adjust', requirePermission('inventory.update'), async (req, res, next) => {
  try {
    ...
    if (!type || !['in', 'out', 'set'].includes(type)) {
      return res.status(400).json({ success: false, message: "type must be 'in', 'out', or 'set'" });
    }
    ...
    const newStock =
      type === 'in'  ? oldStock + qty :
      type === 'out' ? Math.max(0, oldStock - qty) :
      qty;
    const delta = newStock - oldStock;

    const { error: updErr } = await supabase
      .from('products').update({ stock: newStock, updated_at: new Date().toISOString() })
      .eq('id', req.params.id);
    ...
    try {
      await supabase.from('stock_movements').insert([{
        ...
        movement_type: type === 'in' ? 'manual_in' : type === 'out' ? 'manual_out' : 'manual_set',
        quantity: delta,
        old_stock: oldStock,
        new_stock: newStock,
    ...
    await createAuditLog({
      ...
      details: { type, quantity: qty, old_stock: oldStock, new_stock: newStock, notes: notes || null }
    });

    res.json({
      success: true,
      data: { product_id: product.id, old_stock: oldStock, new_stock: newStock, delta }
    });
  } catch (err) {
    next(err);
  }
