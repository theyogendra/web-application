...
const SELECT_LIST = "*";
const SELECT_FULL =
  "*, proposal_items(*), quotations!converted_to_quotation_id(id, quotation_number, status, invoices!converted_to_invoice_id(id, invoice_number, status))";
...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (search)
      q = q.or(
        `proposal_number.ilike.*${search}*,customer_name.ilike.*${search}*`,
      );
  ...
  async (req, res, next) => {
    try {
      ...
      const proposalNumber =
        req.body.proposal_number || (await generateProposalNumber());
      ...
      try {
        ...
        if (convResult && convResult.success) {
          ({ data: quotation } = await supabase
            .from("quotations")
            .select("*")
            .eq("id", convResult.quotation_id)
            .maybeSingle());
          ...
          await createAuditLog({
            req,
            action: "proposal_converted_to_quotation",
            module: "Proposals",
            entityType: "proposal",
            entityId: result.proposal_id,
            details: {
              quotation_id: convResult.quotation_id,
              quotation_number: convResult.quotation_number,
              auto: true,
            },
          });
        }
    ...
    try {
      ...
      if (["converted", "cancelled"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: `Cannot edit a ${existing.status} proposal`,
          });
      }
        ...
        await createAuditLog({
          ...
          details: {
            reason: result.hard_deleted
              ? "parent proposal deleted"
              : "parent proposal cancelled",
    ...
    try {
      ...
      if (!row.customer_email) {
        return res
          ...
          .json({
            success: false,
            message: "Proposal has no customer email address",
          });
      }
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message:
            "Failed to generate the proposal PDF. The proposal was not sent.",
      ...
      if (emailResult.success) {
        ...
        if (Object.keys(patch).length > 0) {
          const { data: u, error: upErr } = await supabase
            ...
            .single();
          if (upErr)
            console.error("Failed to mark proposal as sent:", upErr.message);
          if (u) updated = u;
        }
      }
...
router.get("/:id/pdf", async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${row.proposal_number || "proposal"}.pdf"`,
    );
    res.send(pdfBuffer);
  } catch (err) {
    next(err);
    ...
    try {
      ...
      if (!["draft", "sent"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: `Cannot accept a ${existing.status} proposal`,
          });
      }
    ...
    try {
      ...
      if (!["draft", "sent"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: `Cannot reject a ${existing.status} proposal`,
          });
      }
...
router.post(
  ...
  requirePermission("proposals.convert"),
  async (req, res, next) => {
    try {
      const userId = req.user ? req.user.id : null;
      ...
      const [{ data: quotation }, { data: updatedProposal }] =
        await Promise.all([
          supabase
            .from("quotations")
            .select("*")
