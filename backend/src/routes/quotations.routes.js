...
} = require("../services/numbering.service");
...
const SELECT_LIST = "*";
const SELECT_FULL =
  "*, quotation_items(*), invoices!converted_to_invoice_id(id, invoice_number, status), proposals!converted_from_proposal_id(id, proposal_number, status)";
...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (search)
      q = q.or(
        `quotation_number.ilike.*${search}*,customer_name.ilike.*${search}*`,
      );
  ...
  async (req, res, next) => {
    try {
      ...
      const quotationNumber =
        req.body.quotation_number || (await generateQuotationNumber());
    ...
    try {
      ...
      if (["converted", "cancelled"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: `Cannot edit a ${existing.status} quotation`,
          });
      }
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Update failed" });
      }
...
// Helper: after a quotation RPC returns a cascaded proposal, write the
// matching audit-log entries (primary + backward cascade target).
async function logQuotationTerminationAudit(
  req,
  existing,
  updated,
  result,
  ...
  await createAuditLog({
    ...
    details: cascaded
      ? {
          cascaded_proposal_id: cascaded.id,
          cascaded_proposal_number: cascaded.proposal_number,
        }
      : null,
  });
  if (cascaded) {
    await createAuditLog({
      req,
      action: "proposal_cascade_rejected",
      module: "Proposals",
      entityType: "proposal",
      entityId: cascaded.id,
      details: {
        reason: `linked quotation ${primaryAction.replace("quotation_", "")}`,
...
// Atomic via terminate_quotation RPC (phase 11). Backward cascade: if the
// upstream proposal is still 'converted', mark it rejected in the same tx.
router.delete(
  "/:id",
  ...
  async (req, res, next) => {
    try {
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Terminate failed" });
      }
      ...
      await logQuotationTerminationAudit(
        req,
        existing,
        updated,
        result,
        result.hard_deleted ? "quotation_deleted" : "quotation_cancelled",
      );

      res.json({
        success: true,
        message: result.hard_deleted
          ? "Quotation deleted"
          : "Quotation cancelled",
        data: updated,
    ...
    try {
      ...
      if (!row.customer_email) {
        return res
          ...
          .json({
            success: false,
            message: "Quotation has no customer email address",
          });
      }
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message:
            "Failed to generate the quotation PDF. The quotation was not sent.",
      ...
      if (emailResult.success) {
        ...
        if (Object.keys(patch).length > 0) {
          const { data: u, error: upErr } = await supabase
            ...
            .single();
          if (upErr)
            console.error("Failed to mark quotation as sent:", upErr.message);
          if (u) updated = u;
        }
      }
...
router.get("/:id/pdf", async (req, res, next) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${row.quotation_number || "quotation"}.pdf"`,
    );
    res.send(pdfBuffer);
  } catch (err) {
    next(err);
    ...
    try {
      ...
      if (!["draft", "sent"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: `Cannot accept a ${existing.status} quotation`,
          });
      }
      ...
      let invoice = null;
      try {
        const { data: convResult, error: convErr } = await supabase.rpc(
        ...
        if (convResult && convResult.success) {
          ...
          await createAuditLog({
            ...
            details: {
              invoice_id: convResult.invoice_id,
              invoice_number: convResult.invoice_number,
              auto: true,
            },
          });
        }
...
// Atomic via mark_quotation_rejected RPC (phase 11). Backward cascade in the
  ...
  async (req, res, next) => {
    try {
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Reject failed" });
      }
      ...
      res.json({ success: true, data: row, cascaded: result.cascaded });
    } catch (err) {
      next(err);
    }
  },
  ...
  async (req, res, next) => {
    try {
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Conversion failed" });
      }
