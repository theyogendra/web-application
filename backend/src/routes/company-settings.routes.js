...
const FIELDS = [
  ...
  'tax_number', 'invoice_prefix', 'currency', 'default_terms', 'default_notes'
];

// GET /company-settings  -- the single settings row (created if missing)
router.get('/', async (req, res, next) => {
  try {
    let { data, error } = await supabase.from('company_settings').select('*').limit(1).single();
  ...
  try {
    ...
    if (existing) {
      const upd = await supabase
        .from('company_settings').update(patch).eq('id', existing.id).select().single();
      if (upd.error) throw upd.error;
      data = upd.data;
    } else {
      const ins = await supabase.from('company_settings').insert([patch]).select().single();
    ...
    await createAuditLog({
      ...
      entityType: 'company_settings', entityId: data.id, newData: data
    });
