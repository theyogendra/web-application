...
} = require("../services/invoice-validation.service");
...
const isTransientNetworkError = (err) => {
  ...
  const code = String(
    err.code || (err.cause && err.cause.code) || "",
  ).toLowerCase();
  ...
  try {
    ...
    try {
      ...
      if (result.error && isTransientNetworkError(result.error)) {
        console.warn(
          "[BACKEND] Transient Supabase network error on GET /api/invoices, retrying (1/1)...",
        );
        result = await buildQuery();
      }
    } catch (err) {
      if (isTransientNetworkError(err)) {
        console.warn(
          "[BACKEND] Transient fetch exception on GET /api/invoices, retrying (1/1)...",
        );
        result = await buildQuery();
      } else {
        throw err;
...
router.get("/:id", async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from("invoices")
      .select(
        "*, invoice_items(*), payments(*), customers(name, email, billing_address, gst_number), vendors(name, email, billing_address, gst_number), quotations!converted_from_quotation_id(id, quotation_number, status, proposals!converted_from_proposal_id(id, proposal_number, status))",
      )
      .eq("id", req.params.id)
      .single();
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // Run Validation
      const validation = await validateInvoicePayload(req.body, {
        isUpdate: false,
      });
      ...
      const { subtotal, discount_amount, tax_amount, grand_total } =
        validation.calculatedTotals;
      ...
      const invoiceInput = {
        ...
        sales_person: req.body.sales_person || null,
        internal_notes: req.body.internal_notes || null,
        shipping_charges: req.body.shipping_charges || 0,
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Create failed" });
      }
    ...
    try {
      ...
      if (["approved", "paid", "cancelled"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: "Cannot edit an invoice in this status",
          });
      }
      ...
      const invoicePatch = {
        invoice_number:
          req.body.invoice_number !== undefined
            ...
            : undefined,
        customer_id:
          req.body.customer_id !== undefined ? req.body.customer_id : undefined,
        vendor_id:
          req.body.vendor_id !== undefined ? req.body.vendor_id : undefined,
        customer_name:
        ...
        customer_phone:
          req.body.customer_phone !== undefined
            ...
            : undefined,
        billing_address:
          req.body.billing_address !== undefined
        ...
        shipping_address:
          req.body.shipping_address !== undefined
            ...
            : undefined,
        currency:
          req.body.currency !== undefined ? req.body.currency : undefined,
        sales_person:
          req.body.sales_person !== undefined
            ? req.body.sales_person
            : undefined,
        internal_notes:
          req.body.internal_notes !== undefined
            ...
            : undefined,
        shipping_charges:
          req.body.shipping_charges !== undefined
            ? req.body.shipping_charges
            : undefined,
        additional_charges:
          req.body.additional_charges !== undefined
            ? req.body.additional_charges
            : undefined,
        attachments:
          req.body.attachments !== undefined ? req.body.attachments : undefined,
      ...
      const { data: updateResult, error: updateRpcError } = await supabase.rpc(
      ...
      if (!updateResult || !updateResult.success) {
        return res
          .status(400)
          .json(updateResult || { success: false, message: "Update failed" });
      }
      const { data: invoice } = await supabase
        .from("invoices")
        .select("*")
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const validation = await validateInvoicePayload(payload, {
        isUpdate: true,
        currentInvoiceId: id,
      });
      const validationStatus = validation.isValid ? "passed" : "failed";
    ...
    try {
      ...
      if (!["draft", "needs_review"].includes(existing.status)) {
        return res
          ...
          .json({
            success: false,
            message: "Only draft or needs_review invoices can be submitted",
          });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      const productIds = Array.from(
        new Set(
          invoice.invoice_items.map((it) => it.product_id).filter(Boolean),
        ),
      );
      ...
      for (const item of invoice.invoice_items) {
        ...
        const available = stockById.has(item.product_id)
          ? stockById.get(item.product_id)
          : 0;
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const { data, error } = await supabase.rpc("reserve_invoice_stock", {
        invoice_id_input: id,
      ...
      if (!data || !data.success) {
        return res
          .status(400)
          .json(
            data || { success: false, message: "Stock reservation failed" },
          );
      }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const { data, error } = await supabase.rpc("confirm_invoice_stock", {
        invoice_id_input: id,
        user_id_input: userId,
      });
      if (error) throw error;
      if (!data || !data.success) {
        return res
          .status(400)
          .json(
            data || { success: false, message: "Stock confirmation failed" },
          );
      }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const { data, error } = await supabase.rpc("release_invoice_stock", {
        invoice_id_input: id,
        user_id_input: userId,
      });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const { data, error } = await supabase.rpc("restore_invoice_stock", {
        invoice_id_input: id,
        user_id_input: userId,
      });
  ...
  async (req, res, next) => {
    try {
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Cancel failed" });
      }
      ...
      if (cascaded) {
        await createAuditLog({
          req,
          action: "quotation_cascade_rejected",
          module: "Quotations",
          entityType: "quotation",
          entityId: cascaded.id,
          details: {
            reason: "linked invoice cancelled",
            invoice_id: id,
    ...
    try {
      ...
      if (!["submitted", "draft"].includes(invoice.status)) {
        return res.status(400).json({
          success: false,
          message:
            "Invoice must be in draft or submitted status to request approval",
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      console.log(
        `[invoice/approve] id=${id} status=${invoice.status} user=${userId}`,
      );
  ...
  async (req, res, next) => {
    try {
      ...
      if (!result || !result.success) {
        ...
        return res
          .status(code)
          .json(result || { success: false, message: "Delete failed" });
      }
      ...
      if (cascaded) {
        await createAuditLog({
          ...
          entityId: cascaded.id,
          details: {
            reason: result.hard_deleted
              ? "linked invoice deleted"
    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message:
            "Failed to generate the invoice PDF. The invoice was not sent.",
      ...
      if (emailResult.success) {
        const patch = {};
        if (["draft", "needs_review", "submitted"].includes(invoice.status))
          patch.status = "sent";
        ...
        if (Object.keys(patch).length > 0) {
          const { data: u, error: upErr } = await supabase
            ...
            .single();
          if (upErr)
            console.error("Failed to mark invoice as sent:", upErr.message);
          if (u) updated = u;
        }
      }
  ...
  async (req, res, next) => {
    try {
      ...
      await createAuditLog({
        ...
        entityType: "invoice",
        entityId: invoice.id,
        details: { email_status: emailResult },
      });
