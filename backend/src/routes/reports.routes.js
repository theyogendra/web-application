...
const logView = (req, name) =>
  createAuditLog({
    ...
    module: "Reports",
    details: { report: name, filters: req.query },
  });
...
router.get("/summary", async (req, res, next) => {
  try {
    ...
    logView(req, "summary").catch((err) =>
      console.error("Failed to log summary view:", err.message),
    );
  ...
  try {
    ...
    if (type === "summary") {
      ...
      columns = [
        ...
        {
          label: "Invoice",
          value: (x) => x.invoices && x.invoices.invoice_number,
        },
        {
          label: "Customer",
          value: (x) => x.invoices && x.invoices.customer_name,
        },
      ...
      return res
        .status(400)
        .json({ success: false, message: `Unknown report type: ${type}` });
    }
    ...
    createAuditLog({
      ...
      console.error("Failed to log report export:", err.message),
    );

    const dateTag = new Date().toISOString().slice(0, 10); // YYYY-MM-DD

    if (format === "pdf") {
      const pdf = await generateTablePdf(title, columns, rows, filters);
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${type}-report-${dateTag}.pdf"`,
      );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${type}-report-${dateTag}.csv"`,
    );
    res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
    res.send(csv);
  } catch (err) {
