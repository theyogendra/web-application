    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message:
            "Failed to generate the invoice PDF. The email was not sent.",
      ...
      res.status(status).json({
        ...
        message:
          result.message ||
          (result.success ? "Invoice email sent" : "Email delivery failed"),
        email: result,
      });
    } catch (err) {
      next(err);
  ...
  async (req, res, next) => {
    try {
      ...
      if (!payment_id)
        return res
          .status(400)
          .json({ success: false, message: "payment_id is required" });
      ...
      if (!invoice)
        return res
          .status(404)
          .json({ success: false, message: "Linked invoice not found" });
      if (!invoice.customer_email) {
        return res
          .status(400)
          .json({
      ...
      res.status(status).json({
        ...
        message:
          result.message ||
          (result.success ? "Receipt email sent" : "Email delivery failed"),
        email: result,
      });
    } catch (err) {
      next(err);
