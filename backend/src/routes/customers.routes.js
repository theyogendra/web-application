...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (error || !data) return res.status(404).json({ success: false, message: 'Customer not found' });
    res.json({ success: true, data });
  } catch (err) {
    next(err);
  }
...
router.post('/', async (req, res, next) => {
  try {
    ...
    await createAuditLog({
      req, action: 'customer_created', module: 'Settings',
      entityType: 'customer', entityId: data.id, newData: data
    });
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const { data, error } = await supabase
      .from('customers').update(patch).eq('id', req.params.id).select().single();
    if (error || !data) return res.status(404).json({ success: false, message: 'Customer not found' });

    await createAuditLog({
      req, action: 'customer_updated', module: 'Settings',
      entityType: 'customer', entityId: data.id, newData: data
    });
