...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (payment_method)
      q = q.eq("payment_method", normalizeMethod(payment_method));
...
// GET /payments/export  -- PDF (registered before /:id so it is not shadowed)
  ...
  try {
    ...
    const pdf = await generateTablePdf(
      ...
      [
        ...
        {
          label: "Invoice",
          value: (r) => r.invoices && r.invoices.invoice_number,
        },
        {
          label: "Customer",
          value: (r) => r.invoices && r.invoices.customer_name,
        },
  ...
  async (req, res, next) => {
    try {
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Approval failed" });
      }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const userId = req.user ? req.user.id : null;
      const reason =
        req.body && req.body.reason ? String(req.body.reason) : "Rejected";
      ...
      if (!result || !result.success) {
        return res
          .status(400)
          .json(result || { success: false, message: "Rejection failed" });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.body.payment_method !== undefined)
        patch.payment_method = normalizeMethod(req.body.payment_method);
      if (req.body.payment_date !== undefined)
        patch.payment_date = req.body.payment_date;
      if (req.body.reference_number !== undefined)
        patch.reference_number = req.body.reference_number;
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      const { data: recalc } = await supabase.rpc(
