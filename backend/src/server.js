...
app.use(
  helmet({
    hsts: ENABLE_HTTPS ? { maxAge: 15552000, includeSubDomains: true } : false, // don't send HSTS over plain HTTP, browsers reject it anyway
  }),
);
...
if (require.main === module) {
  (async () => {
    ...
    server.listen(PORT, "0.0.0.0", () => {
      console.log("=========================================");
      console.log(
        `Node backend running on ${scheme.toUpperCase()} port ${PORT}`,
      );
      console.log(`Health check: ${scheme}://localhost:${PORT}/health`);
      if (ENABLE_HTTPS) {
        console.log(
          "Self-signed cert — browser will warn once, accept and reload.",
        );
      }
      console.log("=========================================");
    });
